...
export async function makeClaim(request, response, next) {
    try {
        ...
        const claimPicture = uploadedFile?.secure_url ||
            uploadedFile?.path ||
...
export async function getClaims(_request, response, next) {
    try {
        const claims = await Claim.find()
            ...
            .sort({ createdAt: -1 });
        return response.json(claims.map((claim) => ({
...
export async function updateClaimStatus(request, response, next) {
    try {
        const { status } = request.body;
        if (status !== "approved" && status !== "rejected") {
            return response.status(400).json({ message: "Status must be approved or rejected." });
        }
        const claim = await Claim.findByIdAndUpdate(request.params.id, { status }, { new: true, runValidators: true });
        if (!claim) {
...
export async function analyzeClaim(request, response, next) {
    try {
