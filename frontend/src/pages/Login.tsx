  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'signup') {
      if (!formData.name.trim() || !formData.registrationNo.trim()) {
        setErrorMsg('Please enter your full name and university registration number.');
        return;
      }
    ...
    try {
      ...
      const user = data.data?.user || {
        name: formData.name || (formData.email.includes('admin') ? 'Security Officer' : 'Student'),
        email: formData.email,
        registrationNo: formData.registrationNo || 'REG-USER',
        role: formData.role || (formData.email.includes('admin') ? 'admin' : 'student')
      };

      const finalRole = user.role || (mode === 'signup' ? formData.role : 'student');
      ...
      setSuccessMsg(mode === 'signup' ? 'Account created successfully! Redirecting...' : 'Signed in successfully! Redirecting...');
    ...
    <div className="min-h-screen bg-[#ef7d00] bg-linear-to-b from-[#f37021] via-[#ef7d00] to-[#e65c00] flex flex-col justify-center py-10 sm:px-6 lg:px-8 font-sans text-slate-800">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md px-4 sm:px-0">
        ...
        <p className="text-center text-xs font-medium text-white/90 drop-shadow-xs mt-6">
          Lovely Professional University • Campus Security & Custody Administration
        </p>
      </div>
